...
- **실시간 알림**: 서비스 워커(`sw.js`)를 통해 입출금 시 **실제 기기 알림(잠금화면/알림센터)**을 띄우고, 지원 기기에서는 짧은 진동(Vibration API)도 함께 울립니다. 인앱 토스트 알림은 항상 표시됩니다.
  - **iPhone(Safari)**: 반드시 앱을 **홈 화면에 추가**한 뒤 그 아이콘으로 실행해야 알림이 동작합니다(iOS 16.4+). 일반 Safari 탭에서는 iOS 자체 제약으로 알림을 띄울 수 없어요.
  - 앱이 완전히 종료된 상태(포그라운드/백그라운드 모두 아님)에서는 알림이 오지 않습니다 — 서버 없이 브라우저에서만 동작하는 데모라 실제 은행 앱처럼 원격 푸시는 지원하지 않습니다.
...
1. 새 GitHub 저장소를 만들고 이 폴더의 파일들(`index.html`, `sw.js`, `manifest.json`, `icon.svg`)을 그대로 업로드(푸시)합니다.
   ```bash
   git init
   git add .
...
- iPhone에서 **실제 알림**을 받으려면 공유 버튼 → **홈 화면에 추가**로 앱을 설치한 뒤, 그 아이콘으로 열어서 알림 벨을 눌러 권한을 허용해주세요.
